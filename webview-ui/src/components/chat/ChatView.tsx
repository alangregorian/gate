...
const ChatView = ({ isHidden, showAnnouncement, hideAnnouncement, showHistoryView }: ChatViewProps) => {
	const { version, muxMessages: messages, taskHistory, apiConfiguration, telemetrySetting } = useExtensionState()

	//const task = messages.length > 0 ? (messages[0].say === "task" ? messages[0] : undefined) : undefined) : undefined
	const task = useMemo(() => messages.at(0), [messages]) // leaving this less safe version here since if the first message is not a task, then the extension is in a bad state and needs to be debugged (see MUX.abort)
	...
	const lastApiReqTotalTokens = useMemo(() => {
		const getTotalTokensFromApiReqMessage = (msg: MUXMessage) => {
			...
			const { tokensIn, tokensOut, cacheWrites, cacheReads }: MUXApiReqInfo = JSON.parse(msg.text)
	...
	const [muxAsk, setMUXAsk] = useState<MUXAsk | undefined>(undefined)
				...
				case "ask":
					...
					switch (lastMessage.ask) {
						...
						case "mistake_limit_reached":
							...
							setMUXAsk("mistake_limit_reached")
							setEnableButtons(true)
						...
						case "auto_approval_max_req_reached":
							setSendingDisabled(true)
							setMUXAsk("auto_approval_max_req_reached")
							setEnableButtons(true)
						...
						case "tool":
							...
							const tool = JSON.parse(lastMessage.text || "{}") as MUXSayTool
							switch (tool.tool) {
						...
						case "browser_action_launch":
							setSendingDisabled(isPartial)
							setMUXAsk("browser_action_launch")
							setEnableButtons(!isPartial)
							setPrimaryButtonText("Approve")
							setSecondaryButtonText("Reject")
							break
						...
						case "resume_completed_task":
							setSendingDisabled(false)
							setMUXAsk("resume_completed_task")
							setEnableButtons(true)
							setPrimaryButtonText("Start New Task")
							setSecondaryButtonText(undefined)
	...
	const isStreaming = useMemo(() => {
		const isLastAsk = !!modifiedMessages.at(-1)?.ask // checking muxAsk isn't enough since messages effect may be called again for a tool for example, set muxAsk to its value, and if the next message is not an ask then it doesn't reset. This is likely due to how much more often we're updating messages as compared to before, and should be resolved with optimizations as it's likely a rendering bug. but as a final guard for now, the cancel button will show if the last message is not an ask
		const isToolCurrentlyAsking = isLastAsk && muxAsk !== undefined && enableButtons && primaryButtonText !== undefined
		...
		return false
	}, [modifiedMessages, muxAsk, enableButtons, primaryButtonText])
	...
	const handleSendMessage = useCallback(
		...
		[messages.length, muxAsk, activeQuote],
	)
	...
	/*
	This logic depends on the useEffect[messages] above to set muxAsk, after which buttons are shown and we then send an askResponse to the extension.
	*/
	const handlePrimaryButtonClick = useCallback(
		async (text?: string, images?: string[]) => {
			...
			switch (muxAsk) {
				...
				case "resume_completed_task":
					...
					break
				case "new_task":
					console.info("new task button clicked!", { lastMessage, messages, muxAsk, text })
		...
		[muxAsk, startNewTask, lastMessage],
	)
	...
	const handleSecondaryButtonClick = useCallback(
		...
		[muxAsk, startNewTask, isStreaming],
	)
	...
	const visibleMessages = useMemo(() => {
		return modifiedMessages.filter((message) => {
			switch (message.ask) {
				case "completion_result":
					// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if mux wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
					if (message.text === "") {
						return false
					}
					break
			...
			switch (message.say) {
				...
				case "deleted_api_reqs": // aggregated api_req metrics from deleted messages
					return false
				case "text":
					// Sometimes mux returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
	...
	const isBrowserSessionMessage = (message: MUXMessage): boolean => {
	...
	const groupedMessages = useMemo(() => {
		const result: (MUXMessage | MUXMessage[])[] = []
		let currentGroup: MUXMessage[] = []
			...
			if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
				...
				if (isBrowserSessionMessage(message)) {
					...
					if (message.say === "browser_action") {
						const browserAction = JSON.parse(message.text || "{}") as MUXSayBrowserAction
	...
	const itemContent = useCallback(
		(index: number, messageOrGroup: MUXMessage | MUXMessage[]) => {
