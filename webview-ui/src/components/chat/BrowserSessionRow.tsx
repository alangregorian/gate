...
import { BrowserAction, BrowserActionResult, MUXMessage, MUXSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: MUXMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: MUXMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: MUXMessage[] = []
		let nextActionMessages: MUXMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					<>{isAutoApproved ? "MUX is using the browser:" : "MUX wants to use the browser:"}</>
				</span>
			</div>
			<div
				style={{
