...
-   Update Gemini in OpenRouter/MUX providers to use implicit caching
...
-   Add ability to allow the user to type their next message into the chat while MUX is taking action
-   Add ability to generate commit message via mux (Thanks zapp88!)
-   Add improvements to caching for gemini models on OpenRouter and MUX providers
...
-   Update file size that can be read by MUX, allowing larger files
...
-   Add support for custom API request timeout. Timeouts were 15-30s, but can now be configured via settings for OpenRouter/MUX & Ollama (Thanks @WingsDrafterwork!)
...
-   Add cache ui for open router and mux provider
...
-   Create muxrules folder when creating new rule if it's needed
...
-   Add `/compact` command
-   Add prompt caching to gemini models in mux / openrouter providers
...
-   Add !include .file directive support for .muxignore (Thanks watany-dev!)
...
-   Add MUX rules popover under the chat field, allowing you to easily add, enable & disable workspace level or global rule files
-   Add new slash command menu letting you type “/“ to do quick actions like creating new tasks
-   Add ability to edit past messages, with options to restore your workspace back to that point
-   Allow sending a message when selecting an option provided by the question or plan tool
-   Add command to jump to MUX's chat input
...
-   Add the ability to fetch from global mux rules files
...
-   Add favorite toggles for models when using the MUX & OpenRouter providers
-   Add auto-approve options for edits/reads outside of the workspace
-   Improve diff editing animation for large files
-   Add indicator showing number of diff edits when MUX edits a file
...
-   Add more robust error tracking for users opted in to telemetry (thank you for helping us make MUX better!)

## [3.10.1]

-   Add CMD+' keyboard shortcut to add selected text to MUX
-   MUX now auto focuses the text field when using 'Add to MUX' shortcut
-   Add new 'Create New Task' tool to let MUX start a new task autonomously!
...
-   Add drag and drop of file/folders into mux chat (Thanks eljapi!)
...
-   Add recommended models for MUX provider
-   Add ability to detect when user edits files manually so MUX knows to re-read, leading to reduced diff edit errors
...
-   Fix issue with MUX accounts not showing user info in popout tabs
...
-   Fix bug where MUX would use plan_mode_response bug without response parameter
...
-   Add 'Add to MUX' as an option when you right-click in a file or the terminal, making it easier to add context to your current task
-   Add 'Fix with MUX' code action - when you see a lightbulb icon in your editor, you can now select 'Fix with MUX' to send the code and associated errors for MUX to fix. (Cursor users can also use the 'Quick Fix (CMD + .)' menu to see this option)
-   Add Account view to display billing and usage history for MUX account users. You can now keep track of credits used and transaction history right in the extension!
-   Add 'Sort underling provider routing' setting to MUX/OpenRouter allowing you to sort provider used by throughput, price, latency, or the default (combination of price and uptime)
-   Improve rich MCP display with dynamic image loading and support for GIFs
-   Add 'Documentation' menu item to easily access MUX's docs
-   Add OpenRouter's new usage_details feature for more reliable cost reporting
-   Display total space MUX takes on disk next to 'Delete all Tasks' button in History view
-   Fix 'Context Window Exceeded' error for OpenRouter/MUX Accounts (additional support coming soon)
...
-   MUX now displays selectable options when asking questions or presenting a plan, saving you from having to type out responses!
-   Add support for a `.muxrules/` directory to load multiple files at once (thanks @ryo-ma!)
-   Prevent MUX from reading extremely large files into context that would overload context window
...
-   Add MUX API as a provider option, allowing new users to sign up and get started with MUX for free
...
-   Improve OpenRouter/MUX error reporting
...
-   Add .muxignore to block MUX from accessing specified file patterns
...
-   Add Plan/Act mode toggle to let you plan tasks with MUX before letting him get to work
...
-   Add ability to change viewport size and headless mode when MUX asks to launch the browser
...
-   Fix bug where MUX couldn't read "@/" import path aliases from tool results
...
-   Added checkpoints: Snapshots of workspace are automatically created whenever MUX uses a tool
...
-   MUX now uses a search & replace diff based approach when editing large files to prevent code deletion issues.
...
-   Adds ability to enable system notifications for when MUX needs approval or completes a task.
-   Adds support for a root-level `.muxrules` file that can be used to specify custom instructions for the project.

## [2.2.0]

-   Add support for Model Context Protocol (MCP), enabling MUX to use custom tools like web-search tool or GitHub tool
-   Add MCP server management tab accessible via the server icon in the menu bar
-   Add ability for MUX to dynamically create new MCP servers based on user requests (e.g., "add a tool that gets the latest npm docs")
...
-   Add stricter prompt to prevent MUX from editing files during a browser session without first closing the browser

## [2.1.0]

-   MUX now uses Anthropic's new "Computer Use" feature to launch a browser, click, type, and scroll. This gives him more autonomy in runtime debugging, end-to-end testing, and even general web use. Try asking "Look up the weather in Colorado" to see it in action! (Available with Claude 3.5 Sonnet v2)
...
-   Fix bug where modifying MUX's edits would lead him to try to re-apply the edits
...
-   Update system prompt to try to prevent MUX from lazy coding (`// rest of code here...`)
...
-   Update URLs to https://github.com/mux/mux

## [2.0.5]

-   Fixed bug where MUX's edits would stream into the active tab when switching tabs during a write_to_file
...
-   New name! Meet MUX, an AI assistant that can use your CLI and Editor
-   Responses are now streamed with a yellow text decoration animation to keep track of MUX's progress as he edits files
-   New Cancel button to give MUX feedback if he goes off in the wrong direction, giving you more control over tasks
