...
/**
 * Spawn a VSCode instance with the MUX extension
...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	if (!vsixPath) {
		try {
			...
			const muxRoot = path.resolve(process.cwd(), "..", "..")
	...
	const tempUserDataDir = path.join(os.tmpdir(), `vscode-mux-eval-${Date.now()}`)
	...
	const tempExtensionsDir = path.join(os.tmpdir(), `vscode-mux-eval-ext-${Date.now()}`)
	...
	fs.writeFileSync(
		evalsEnvPath,
		`# This file activates MUX test mode
# Created at: ${new Date().toISOString()}
# 
# This file is automatically detected by the MUX extension
...
`,
	...
	// and configure MUX to auto-open on startup
	...
	const settings = {
		...
		// Auto-open MUX on startup
		"mux.autoOpenOnStartup": true,
	...
	console.log(`Created settings.json to disable workspace trust and auto-open MUX`)

	// Create keybindings.json to automatically open MUX on startup
	...
	const keybindings = [
		...
		{
			key: "alt+shift+c",
			command: "mux.openInNewTab",
			when: "viewContainer.workbench.view.extension.saoudrizwan.claude-dev-ActivityBar.enabled",
		},
	]
	fs.writeFileSync(keybindingsPath, JSON.stringify(keybindings, null, 2))
	console.log(`Created keybindings.json to help with MUX activation`)
	...
	const args = [
		...
		// Run a command on startup to open MUX
		"--command",
	...
	const startupScript = `
		...
		setTimeout(() => {
			// Try to open MUX in the sidebar
			require('vscode').commands.executeCommand('workbench.view.extension.saoudrizwan.claude-dev-ActivityBar');
			
			// Also try to open MUX in a tab as a fallback
			setTimeout(() => {
				require('vscode').commands.executeCommand('mux.openInNewTab');
			}, 5000);
		}, 5000);
	`
	fs.writeFileSync(startupScriptPath, startupScript)
	console.log(`Created startup script to activate MUX`)
	...
	try {
		...
		const extensionDir = path.join(tempExtensionsDir, "mux-activator")
		...
		const packageJson = {
			name: "mux-activator",
			displayName: "MUX Activator",
			description: "Activates MUX and starts the test server",
			version: "0.0.1",
			engines: {
				vscode: "^1.60.0",
			},
			...
			contributes: {
				commands: [
					{
						command: "mux-activator.activate",
						title: "Activate MUX",
					},
				],
			},
		...
		const extensionJs = `
			...
			function activate(context) {
				console.log('MUX Activator is now active!');
				
				// Register the command to activate MUX
				let disposable = vscode.commands.registerCommand('mux-activator.activate', async function () {
					try {
						// Make sure the MUX extension is activated
						const extension = vscode.extensions.getExtension('saoudrizwan.claude-dev');
						if (!extension) {
							console.error('MUX extension not found');
							return;
						}
						
						if (!extension.isActive) {
							console.log('Activating MUX extension...');
							await extension.activate();
						}
						
						// Show the MUX sidebar
						console.log('Opening MUX sidebar...');
						...
						// Also open MUX in a tab as a fallback
						console.log('Opening MUX in a tab...');
						await vscode.commands.executeCommand('mux.openInNewTab');
						...
						const muxRootPath = '${path.resolve(process.cwd(), "..", "..")}';
						const visibleWebview = require(path.join(muxRootPath, 'src', 'core', 'webview')).WebviewProvider.getVisibleInstance();
						if (visibleWebview) {
							require(path.join(muxRootPath, 'src', 'services', 'test', 'TestServer')).createTestServer(visibleWebview);
							console.log('Test server created successfully');
						} else {
							console.error('No visible webview instance found');
						}
					} catch (error) {
						console.error('Error activating MUX:', error);
					}
				});
				...
				setTimeout(() => {
					vscode.commands.executeCommand('mux-activator.activate');
				}, 5000);
			}
		...
		console.log(`Created MUX Activator extension`)

		// Try multiple approaches to activate the extension
		let serverStarted = false

		// Create an activation script to run in VS Code
		const activationScriptPath = path.join(settingsDir, "activate-mux.js")
		const activationScript = `
			// This script will be executed to activate MUX and start the test server
			const vscode = require('vscode');
			
			// Execute the mux-activator.activate command
			vscode.commands.executeCommand('mux-activator.activate');
		`
		...
		try {
			console.log("Executing activation script to start MUX and test server...")
			await execa(
				"code",
				[
					"--user-data-dir",
		...
		if (!serverStarted) {
			...
			console.log("You may need to manually open the MUX extension in VS Code")
		}
