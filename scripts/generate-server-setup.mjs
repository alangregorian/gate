...
const muxDef = protoLoader.loadFileDescriptorSetFromBuffer(fs.readFileSync(DESCRIPTOR_SET))
...
function generateHandlersAndExports() {
	...
	for (const [name, def] of Object.entries(proto.mux)) {
		...
		handlerSetup.push(`    server.addService(proto.mux.${name}.service, {`)
