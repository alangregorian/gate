...
const getMUXVersion = () => {
	try {
		...
		const muxMatch = extensions.match(/claude-dev@(\d+\.\d+\.\d+)/)
		return muxMatch ? muxMatch[1] : "Not installed"
	} catch (err) {
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/mux/mux/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&mux-version=${systemInfo.muxVersion}` +
		`&system-info=${
