...
description: "Learn about the two primary transport mechanisms for communication between MUX and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between MUX and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (MUX) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as MUX
...
-   Single-client scenarios (one MUX instance per server)
...
1. The client (MUX) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your MUX instance
...
STDIO servers run locally on the same machine as MUX, which has several important implications:
...
-   **Execution**: Starts and stops with MUX (child process lifecycle)
...
-   Start when needed by MUX
-   Not require network configuration
-   Need to be installed alongside MUX or via a package manager
...
## Configuring Transports in MUX

For detailed information on configuring STDIO and SSE transports in MUX, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).
