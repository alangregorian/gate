...
description: "This protocol is designed to streamline the development process of building MCP servers with MUX."
---

> 🚀 **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [MUX MCP Marketplace](https://github.com/mux/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like MUX by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.muxrules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/MUX/MCP).

### Using `.muxrules` Files

A `.muxrules` file is a special configuration that MUX reads automatically when working in the directory where it's placed. These files:

-   Configure MUX's behavior and enforce best practices
-   Switch MUX into a specialized MCP development mode
...
Here's the complete MCP Server Development Protocol that should be placed in your `.muxrules` file:
...
When this `.muxrules` file is present in your working directory, MUX will:
...
### 1. Create a `.muxrules` file (🚨 IMPORTANT)

First, add a `.muxrules` file to the root of your MCP working directory using the protocol above. This file configures MUX to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your MUX chat by clearly describing what you want to build. Be specific about:
...
MUX will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help MUX build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves MUX's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with MUX to design your MCP server:
...
Once planning is complete, MUX helps implement the server:
...
<Frame>
	<img
		src="https://storage.googleapis.com/mux_public_images/docs/assets/planning-phase.gif"
...
<Frame>
	<img
		src="https://storage.googleapis.com/mux_public_images/docs/assets/building-mcp-plugin.gif"
...
Resources let your MCP servers expose data to MUX without executing code. They're perfect for providing context like files, API responses, or database records that MUX can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing MUX to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
