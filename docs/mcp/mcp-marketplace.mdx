---
title: "MCP Made Easy"
description: "Learn how to use the MCP Marketplace to discover, install, and configure MCP servers that enhance MUX's capabilities with additional tools and resources."
---

## What's an MCP Server?

MCP servers are specialized extensions that enhance MUX's capabilities. They enable MUX to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In MUX, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), MUX will guide you through:
...
-   MUX will show confirmation when installation is complete
-   Check the server status in MUX's MCP settings UI

### 5. Using Your New Server

-   After successful installation, MUX will automatically integrate the server's capabilities
-   You'll see new tools and resources available in MUX's system prompt
-   Simply ask MUX to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using MUX in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   The server code is cloned/installed to `/Users/<username>/Documents/MUX/MCP/`
...
-   This file is located at: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/mux_mcp_settings.json`
-   Environment variables (like API keys) are securely stored
-   The server path is registered

### 3. Server Launch

-   MUX detects the configuration change
-   MUX launches your server as a separate process
-   Communication is established via stdio or HTTP

### 4. Integration with MUX

-   Your server's capabilities are added to MUX's system prompt
-   Tools become available via `use_mcp_tool` commands
-   Resources become available via `access_mcp_resource` commands
-   MUX can now use these capabilities when prompted by the user
...
1. Open the MCP settings file: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/mux_mcp_settings.json`
...
-   Try removing and reinstalling the server (remove from both the `mux_mcp_settings.json` file and the `/Users/<username>/Documents/MUX/MCP/` directory)
...
MUX is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.

Utilize a `.muxrules` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making MUX's tool selection more intuitive and context-aware.

### How MCP Rules Work

MCP Rules group your connected MCP servers into functional categories and define trigger keywords that activate them automatically when detected in your conversations with MUX.
...
-   **Contextual Tool Selection**: MUX selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", MUX detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
...
Add this to your `.muxrules` file or to your custom instructions to make MUX's MCP server selection more intuitive and context-aware.
