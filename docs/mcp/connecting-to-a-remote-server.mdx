...
description: "The Model Context Protocol (MCP) allows MUX to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
To access the MCP Servers interface in MUX:

1. Click on the MUX icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the MUX panel
...
4. MUX will attempt to connect to the server and display the connection status
...
Smithery is a third-party MCP server marketplace that allows users to discover and connect to a variety of Model Context Protocol (MCP) servers. If you're using an MCP-compatible client (such as Cursor, Claude Desktop, or MUX), you can browse available servers and integrate them directly into your workflow.
...
2. **Request Timeout**:
    - Set how long MUX should wait for server responses
...
-   **Enabled**: MUX can use the server's tools and resources
...
For advanced users, MUX stores MCP server configurations in a JSON file that can be modified:
...
Once connected, MUX can use the tools and resources provided by the MCP server. When MUX suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to MUX
