...
Think of **custom instructions as MUX's programming**. They define MUX's baseline behavior and are **always "on," influencing all interactions.** Instructions can be broad and abstract, or specific and explicit. You might want MUX to have a unique personality, or produce output in a particular file format, or adhere to certain architectural principles. Custom instructions can standardize MUX's output in ways you define, which is especially valuable when working with others. See the [Enterprise section](../enterprise-solutions/custom-instructions.md) for using Custom Instructions in a team context.\
\

<mark style="color:yellow;">
	NOTE: Modifying the Custom Instructions field updates MUX's prompt cache, discarding accumulated context. This causes a
...
<Frame>
	<img src="https://storage.googleapis.com/mux_public_images/docs/assets/image%20(1).png" alt="MUX Logo" />
</Frame>
...
<mark style="color:yellow;">`.muxrules`</mark>
<mark style="color:yellow;">
	file updates MUX's prompt cache, discarding accumulated context. This causes a temporary increase in cost while that context
	is replaced. Update the
</mark> <mark style="color:yellow;"></mark>
<mark style="color:yellow;">`.muxrules`</mark> <mark style="color:yellow;"></mark>
...
### .muxrules Folder System 📂

While a single `.muxrules` file works well for simpler projects, MUX now supports a `.muxrules` folder for more sophisticated rule organization. This modular approach brings several advantages:

#### How It Works

Instead of a single file, create a `.muxrules/` directory in your project root:

```
your-project/
├── .muxrules/              # Folder containing active rules
...
MUX automatically processes **all Markdown files** inside the `.muxrules/` directory, combining them into a unified set of rules. The numeric prefixes (optional) help organize files in a logical sequence.
...
```
your-project/
├── .muxrules/              # Active rules - automatically applied
│   ├── 01-coding.md
│   └── client-a.md
│
├── muxrules-bank/          # Repository of available but inactive rules
...
rm .muxrules/client-a.md
cp muxrules-bank/clients/client-b.md .muxrules/
```
...
cp muxrules-bank/frameworks/react.md .muxrules/
```
...
-   Consider git-ignoring the active `.muxrules/` folder while tracking the `muxrules-bank/`
-   Create team scripts to quickly activate common rule combinations

The folder system transforms your MUX rules from a static document into a dynamic knowledge system that adapts to your team's changing contexts and requirements.

### Managing Rules with the Toggleable Popover

To make managing both single `.muxrules` files and the folder system even easier, MUX v3.13 introduces a dedicated popover UI directly accessible from the chat interface.

Located conveniently under the chat input field, this popover allows you to:

-   **Instantly See Active Rules:** View which global rules (from your user settings) and workspace rules (`.muxrules` file or folder contents) are currently active.
-   **Quickly Toggle Rules:** Enable or disable specific rule files within your workspace `.muxrules/` folder with a single click. This is perfect for activating context-specific rules (like `react-rules.md` or `memory-bank.md`) only when needed.
-   **Easily Add/Manage Rules:** Quickly create a workspace `.muxrules` file or folder if one doesn't exist, or add new rule files to an existing folder.
...
## .muxignore File Guide

### Overview

The `.muxignore` file is a project-level configuration file that tells MUX which files and directories to ignore when analyzing your codebase. Similar to `.gitignore`, it uses pattern matching to specify which files should be excluded from MUX's context and operations.

### Purpose

-   **Reduce Noise**: Exclude auto-generated files, build artifacts, and other non-essential content
-   **Improve Performance**: Limit the amount of code MUX needs to process
-   **Focus Attention**: Direct MUX to relevant parts of your codebase
-   **Protect Sensitive Data**: Prevent MUX from accessing sensitive configuration files
