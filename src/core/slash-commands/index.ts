...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsMUXrulesFileCheck: boolean }> {
	...
	for (const { tag, regex } of tagPatterns) {
		...
		if (match) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsMUXrulesFileCheck: commandName === "newrule" ? true : false }
			}
			...
			if (matchingWorkflow) {
				try {
					...
					return { processedText, needsMUXrulesFileCheck: false }
				} catch (error) {
	...
	return { processedText: text, needsMUXrulesFileCheck: false }
}
