...
export function registerTaskCommands(context: vscode.ExtensionContext, controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("mux.dev.createTestTasks", async () => {
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): MUXMessage[] {
	...
	const messages: MUXMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in MUX.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
