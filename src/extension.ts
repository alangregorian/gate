...
import { createMUXAPI } from "./exports"
...
export function activate(context: vscode.ExtensionContext) {
	...
	Logger.log("MUX extension activated")
	...
	vscode.commands.executeCommand("setContext", "mux.isDevMode", IS_DEV && IS_DEV === "true")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.mcpButtonClicked", (webview: any) => {
	...
	const openMUXInNewTab = async () => {
		Logger.log("Opening MUX in new tab")
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "MUX", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(vscode.commands.registerCommand("mux.popoutButtonClicked", openMUXInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("mux.openInNewTab", openMUXInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("mux.settingsButtonClicked", (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.historyButtonClicked", (webview: any) => {
			WebviewProvider.getAllInstances().forEach((instance) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.accountButtonClicked", (webview: any) => {
			WebviewProvider.getAllInstances().forEach((instance) => {
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				...
				Logger.log("MUX dev task commands registered")
			})
			.catch((error) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					const addAction = new vscode.CodeAction("Add to MUX", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: "mux.addToChat",
						title: "Add to MUX",
						arguments: [expandedRange, context.diagnostics],
					}

					const fixAction = new vscode.CodeAction("Fix with MUX", vscode.CodeActionKind.QuickFix)
					fixAction.command = {
						command: "mux.fixWithMUX",
						title: "Fix with MUX",
						arguments: [expandedRange, context.diagnostics],
					}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.fixWithMUX", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			// Add this line to focus the chat input first
			await vscode.commands.executeCommand("mux.focusChatInput")
			...
			const visibleWebview = WebviewProvider.getVisibleInstance()
			await visibleWebview?.controller.fixWithMUX(selectedText, filePath, languageId, diagnostics)
		}),
	)

	// Register the focusChatInput command handler
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.focusChatInput", () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("mux.generateGitCommitMessage", async () => {
			...
			if (controller) {
				...
				const outputChannel = vscode.window.createOutputChannel("MUX Commit Generator")
	...
	return createMUXAPI(outputChannel, sidebarWebview.controller)
}
...
export async function deactivate() {
	...
	Logger.log("MUX extension deactivated")
}
