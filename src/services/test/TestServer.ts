...
import { MUXAsk, ExtensionMessage } from "@shared/ExtensionMessage"
import { ApiProvider } from "@shared/api"
import { HistoryItem } from "@shared/HistoryItem"
import { getSavedMUXMessages, getSavedApiConversationHistory } from "@core/storage/disk"
...
export function createTestServer(webviewProvider?: WebviewProvider): http.Server {
	// Try to show the MUX sidebar
	Logger.log("[createTestServer] Opening MUX in sidebar...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active MUX instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "mux" as ApiProvider,
							muxApiKey: apiKey,
						}

						// Store the API key securely
						await storeSecret(visibleWebview.controller.context, "muxApiKey", apiKey)

						// Update the API configuration
						await updateApiConfiguration(visibleWebview.controller.context, updatedConfig)

						// Update global state to use mux provider
						await updateGlobalState(visibleWebview.controller.context, "apiProvider", "mux" as ApiProvider)
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedMUXMessages(visibleWebview.controller.context, taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved MUX messages: ${error}`)
						}
...
export function createMessageCatcher(webviewProvider: WebviewProvider): vscode.Disposable {
	Logger.log("MUX message catcher registered")
	...
	if (webviewProvider && webviewProvider.controller) {
		...
		webviewProvider.controller.postMessageToWebview = async (message: ExtensionMessage) => {
			...
			if (message.type === "partialMessage" && message.partialMessage?.type === "ask" && !message.partialMessage.partial) {
				const askType = message.partialMessage.ask as MUXAsk
	...
	return new vscode.Disposable(() => {
		...
		Logger.log("MUX message catcher disposed")
	})
}
 ...
 */
async function autoRespondToAsk(webviewProvider: WebviewProvider, askType: MUXAsk, askText?: string): Promise<void> {
	...
	switch (askType) {
		...
		case "new_task":
			// Demux creating a new task to keep the current task running
			responseType = "messageResponse"
