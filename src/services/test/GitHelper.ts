...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".mux_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "MUX Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "mux@example.com"], { cwd: workspacePath })
