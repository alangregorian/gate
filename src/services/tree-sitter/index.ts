...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = muxIgnoreController ? muxIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, muxIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (muxIgnoreController && !muxIgnoreController.validateAccess(filePath)) {
		return null
	}
