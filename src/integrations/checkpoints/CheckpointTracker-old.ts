...
import { Controller as MUXProvider } from "@core/controller"
import { fileExistsAtPath } from "@utils/fs"
import { globby } from "globby"

class CheckpointTracker {
	private providerRef: WeakRef<MUXProvider>
	...
	private constructor(provider: MUXProvider, taskId: string, cwd: string) {
	...
	private static async getWorkingDirectory(): Promise<string> {
		...
		if (!cwd) {
			throw new Error("No workspace detected. Please open MUX in a workspace to use checkpoints.")
		}
	...
	public static async doesShadowGitExist(taskId: string, provider?: MUXProvider): Promise<boolean> {
	...
	public async initShadowGit(): Promise<string> {
		...
		if (await fileExistsAtPath(gitPath)) {
			...
			await git.addConfig("user.name", "MUX Checkpoint")
